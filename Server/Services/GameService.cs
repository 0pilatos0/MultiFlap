...
namespace Server.Services
{
	public class GameService : IGameService
	{
		...
		private readonly ConcurrentDictionary<string, Match> _matches = new ConcurrentDictionary<string, Match>();
		...
		public async Task RemovePlayerAsync(string connectionId)
		{
			if (_players.TryRemove(connectionId, out var player))
			{
				...
				{
					var opponentId = match.Players.FirstOrDefault(p => p.ConnectionId != connectionId)?.ConnectionId;
		...
		public async Task StartMatchmakingAsync(string connectionId)
		{
			...
			if (_players.TryGetValue(connectionId, out var player))
			{
				...
				else
				{
					...
					await SendToClientsAsync(new List<string> { player.ConnectionId, opponentId }, "MatchStarted", match.Players);
		...
		public async Task CancelMatchmakingAsync(string connectionId)
		{
			if (_players.TryGetValue(connectionId, out var player))
			{
				...
				if (player.MatchId != null && _matches.TryGetValue(player.MatchId, out var match))
				{
					...
					{
						match.Players = match.Players.Where(p => p.ConnectionId != connectionId).ToList();
					}
				}
		...
		private async Task SendToClientAsync(string connectionId, string methodName, object? argument = null)
		{
			await _hubContext.Clients.Client(connectionId).SendAsync(methodName, argument);
		}

		private async Task SendToClientsAsync(IEnumerable<string> connectionIds, string methodName, object? argument = null)
		{
			await _hubContext.Clients.Clients(connectionIds.ToList()).SendAsync(methodName, argument);
		}
		...
		private Match FindMatch(Player player)
		{
			...
			var newMatch = new Match
			{
				...
				Players = new List<PlayerMatchInfo> { new PlayerMatchInfo { ConnectionId = player.ConnectionId, Y = player.Y, Score = player.Score } }
			};
